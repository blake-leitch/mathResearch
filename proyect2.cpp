...
int main() {
    ...
    vector<string> obtained(1000000, "Not Found"); //set all permutations to not found
    for (int i = 10; i <= 1000000; ++i) {
        ...
        for (auto permutation : permutations){ //for every permutation of some number
            if (permutation + i <= 1000000) { //s.t. the swapped sum is less than 1 mil
                obtained[permutation + i] = to_string(permutation + i) + " = " + to_string(i) + " + " + to_string(permutation); //give the swapped sum, and give an example of what number and permutation make the sum.
            }
        }
    }
    ...
    //prints to txt file
    ofstream myfile;
    myfile.open ("possiblesums.txt");
    for (int i = 10; i <= 1000000; ++i) {
            myfile << obtained[i] << endl;
        }
    myfile.close();
